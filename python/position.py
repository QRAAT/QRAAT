...
def get_db_con(db_view):
  try:
    db_config = qraat.csv("%s/db_auth" % os.environ['RMG_SERVER_DIR']).get(view=db_view)
  ...
  return mdb.connect(db_config.host,
                     db_config.user,
                     db_config.password,
                     db_config.name)
...
class cal_data:
  ...
  def __init__(self, db_con, cal_id): 
    ...
    # Get site locations.
    self.sites = qraat.csv(db_con=db_con, db_table='sitelist')

    self.sv_deps_by_site = {}

    for row in self.sites:
      self.deps.append(('sitelist', row.ID))
    ...
    for site in self.sites:
      ...
      self.prov_sv_ids = get_field(raw_data, 0)
      data_no_ids = remove_field(raw_data, 0)
      sv_data = np.array(data_no_ids,dtype=float)
      if sv_data.shape[0] > 0:
        self.steering_vectors[site.ID] = np.array(sv_data[:,1::2] + np.complex(0,1) * sv_data[:,2::2])
        self.sv_deps_by_site[site.ID] = self.prov_sv_ids
        self.bearings[site.ID] = np.array(sv_data[:,0])
      else:
...
class est_data:
  ...
  def __init__ (self, db_con, t_start, t_end, tx_id): 
    ...
    raw_data = cur.fetchall()
    self.prov_est_ids = get_field(raw_data, 0)
    self.signal_data = np.array(raw_data, dtype=float)
    est_ct = self.signal_data.shape[0]
    if est_ct == 0:
      print >>sys.stderr, "position: fatal: no est records for selected time range."
      sys.exit(1)
    else: print "position: processing %d records" % est_ct

    self.sig_id =   np.array(self.signal_data[:,0], dtype=int)
    self.site_id =  np.array(self.signal_data[:,1], dtype=int)
    self.est_time = self.signal_data[:,2]
    self.signal =   self.signal_data[:,3::2]+np.complex(0,-1)*self.signal_data[:,4::2]


  def get_likelihoods(self, cal, record_provenance_from_site_data = False):
    ...
    for i in range(signal.shape[0]):
      ...
      likelihood_deps = {}
      for j, value in enumerate(cal.bearings[site_id[i]]):
