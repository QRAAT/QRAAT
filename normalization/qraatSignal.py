  ...
  def read_db(self, db_con, dep_id, t_start, t_end,
    ...
    if (score_threshold == 8): 
      ct = cur.execute('''SELECT ID, siteID, timestamp, edsp, 
                            ...
                            FROM est
                            JOIN est_bearing2 ON est.ID = est_bearing2.estID
                           WHERE deploymentID= %s
                             AND timestamp >= %s 
                             AND timestamp <= %s
                             AND scaled_expected_bearing_likelihood >= 0.55
                           ORDER BY timestamp''', 
      ...
      ct = cur.execute('''SELECT ID, siteID, timestamp, edsp, 
                                 ...
                                 nc41r, nc41i, nc42r, nc42i, nc43r, nc43i, nc44r, nc44i 
                            FROM est
                            JOIN est_class4 ON est.ID = est_class4.estID
                           WHERE deploymentID= %s
                             AND timestamp >= %s 
                             AND timestamp <= %s
                             AND isPulse = 1
      ...
      ct = cur.execute('''SELECT ID, siteID, timestamp, edsp, 
                                 ...
                                 nc41r, nc41i, nc42r, nc42i, nc43r, nc43i, nc44r, nc44i 
                            FROM est
                            JOIN est_class5 ON est.ID = est_class5.estID
                           WHERE deploymentID= %s
                             AND timestamp >= %s 
                             AND timestamp <= %s
                             AND isPulse = 1
